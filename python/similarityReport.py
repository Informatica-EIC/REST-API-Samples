...
Note:  this is 10.2.1+ only - the method is different starting with 10.2.1 & uses v1 (undocumented) api to get the additional context

Note:  this script is rough - it works, but has not been optimized in any way or comments added
        things to add/change include
            support the catalog paging model - it will only process a single page at the moment (pageSize property)
...
objectsurl = catalogServer + '/access/1/catalog/data/objects'  # note:  v1 api used here for additionalFacts
header = {"Accept": "application/json"} 
# note:  remove the and core.resourceName (or change the resource name to *) - to process all columns (in all resources)
...
columnHeader=["ObjectId", "SimilarObjectId","Confidence","Data Similarity","Frequency Similarity", "Pattern Similarity", "Name Similarity"]
...
while offset<total:
    ...
    total=resultJson['totalCount']  # note v1 total count is not indside of ['metadata']
    print("processing:" + str(offset+1) + "-" + str(offset+pageSize) + "/" + str(total) + " pagesize="+str(pageSize) + " currentPage=" + str(page) );
    ...
    for foundItem in resultJson["items"]:
        ...
        for addFacts in foundItem['additionalFacts']:
            ...
            if provider=='similar-cols-provider':
                ...
                respSim = requests.get(simUrl,params=simParms,headers=header, auth=HTTPBasicAuth(uid,pwd))
                ...
                for dstObject in simResult["dstObjects"]:
                    ...
                    for linkProps in dstObject['linkProperties']:
                        ...
                        if name=="com.infa.ldm.similarity.confidenceScore":
                            scoreSim=val
                    # all progperties processed - print the similar scores to screen and file
                    #print("\t\t" + scoreSim +":" + patternSim +":" + nameSim +":" + valSim +":" + frqSim +":")
                    simLinks += 1
                    colWriter.writerow([itemId,dstId,scoreSim,valSim,frqSim,patternSim,nameSim ])
                if hasSimLinks:
                    itemsWithSim += 1
...
print("counts: items=" + str(itemCount) + " items that have similar links=" + str(itemsWithSim) + " similarLinks=" + str(simLinks))
