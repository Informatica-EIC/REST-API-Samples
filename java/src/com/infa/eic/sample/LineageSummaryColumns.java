...
//		System.out.println(lineageSummary.getIdPart(test, 0).replaceAll(":", ""));
		...
		lineageSummary.run(resourceName, outFolder, fileNamePrefix, edcQuery);
	 ...
	 */
	public void run(String resource, String folder, String prefix, String query) {
...
//										+ getParentNameFromId(or.getId()) + sepa
										+ fromColumnNane + sepa
//										+ objName + sepa
										+ lr.getProviderId() + sepa    // lineage scannerType
//										+ lr.g
										+ linkingResource + sepa								// lineageScannerName
										+ getIdPart(lr.getId(), 0).replaceAll(":", "") + sepa     
										+ lr.getId() + sepa  
										+ lr.getClassType() + sepa  
										+ toSchemaName	+ sepa	// schema name
//										+ getIdPart(lr.getId(), -3)	+ sepa	// schema name
										+ toTableName + sepa   // table name
//										+ getParentNameFromId(lr.getId()) + sepa   // table name
										+ toColumnName	+ sepa	// column name
										+ "\"" + operation + "\"";
...
//									System.out.println("linking resource???" + linkingResource);
							...
							outLine = getIdPart(lr.getId(), 0).replaceAll(":", "") + sepa   
									...
									+ toTableName + sepa   // table name
									+ toColumnName	+ sepa	// column name
									+ lr.getProviderId() + sepa   // lineageScannerType
									+ linkingResource + sepa
									+ APIUtils.getValue(or, "core.resourceName") + sepa
									+ or.getId() + sepa  
									+ fmClassType + sepa  
									+ fromSchemaName + sepa
									+ fromTableName + sepa
...
//														duplicates++;
					...
					columnStats.add(APIUtils.getValue(or, "core.resourceName") 
							+ sepa + or.getId()
							+ sepa + APIUtils.getValue(or, "core.classType")
							+ sepa + getIdPart(or.getId(), -3)
							+ sepa + getIdPart(or.getId(), -2)
							+ sepa + objName
							+ sepa + numFacts 
							+ sepa + numSrcLinks
							+ sepa + numDstLinks
